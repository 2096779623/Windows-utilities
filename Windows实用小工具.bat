...
@echo off
title Windows实用小工具 By 2096779623 v2.0.2.17
color 07
::编码为ANSI!!!
...
:memu
cls
for /f "tokens=3*" %%A in ('reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v "ProductName"') do echo 当前系统:%%A %%B %PROCESSOR_ARCHITECTURE%
echo 程序运行目录:%CD%
...
echo 输入exit退出
echo --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
echo 101.启用.NET 3.5                   111.刷新DNS缓存                           121.使用powershell开关热点                   131.删除windows资源管理器上方的百度网盘
echo 102.禁止一联网就打开浏览器         112.文件/文件夹选项                       122.修复键盘映射                             132.删除windows资源管理器上方的酷狗音乐
echo 103.启用无GUI引导(开机隐藏winlogo) 113.修改按下电源按钮时"无操作"            123.修复QQ登录蓝屏                           133.打开/关闭临时IPV6地址
echo 104.关闭SmartScreen应用筛选器      114.添加按住Ctrl,再按两次ScrollLock键蓝屏 124.清除IE收藏夹                             134.预防autorun病毒(U盘)
echo 105.关机时快速强杀后台增加关机速度 115.修复开始菜单无法打开                  125.禁用磁盘自检
echo 106.直接永久删除文件(不进入回收站) 116.强制更新组策略                        126.重建图标缓存
echo 107.去除UAC小盾牌图标              117.添加一个桌面右键菜单                  127.启用修复模式
echo 108.启用Administrator账户          118.公司网络和互联网同时访问              128.解决Windows 开机logo模糊或者被拉伸的问题
echo 109.打开DHCP                       119.给右键菜单添加图标                    129.管理员取得所有权
echo 110.设置静态IP                     120.插入U盘自动打开Win资源管理器(Win10)   130.获取所有杀毒软件名字以及路径
echo --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
set /p user_input=请输入你要执行的操作：
...
DEL IconCache.db /a
echo 请手动重启windows资源管理器或者重启电脑生效！
pause
cls
goto memu1
